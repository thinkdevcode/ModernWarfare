...
int cpObject::CheckState(const sf::Input *input)
{
	...
	if(CheckMouseEntered(input) && mouseIn)
	{
		outlineColor = sf::Color(167,217,245);
		outlineColorTemp = outlineColor;
	...
	if((CheckMouseEntered(input) && input->IsMouseButtonDown(sf::Mouse::Left)))
	{
...
void cpButton::SetBackgroundColor(sf::Color color)
{
	...
	backgroundColor2 = sf::Color((sf::Uint8)r,(sf::Uint8)g,(sf::Uint8)b);
	backgroundColor2Temp = backgroundColor2;
	CreateRects(Label.GetText());
}
...
int cpButton::CheckState(const sf::Input *input)
{
	...
	{
		backgroundColor2 = sf::Color(167,217,245); //44 98 139
	}
...
void cpTextInputBox::ProcessTextInput(sf::Event *evt)
{
	...
	if (evt->Type == sf::Event::TextEntered)
    {
        ...
        unsigned short unicode = (unsigned short)evt->Text.Unicode;
...
void cpTextInputBox::SetFont(std::string filename)
{
...
int cpSelectionBox::CheckState(const sf::Input *input)
{
	int state = cpObject::CheckState(input);
	sf::Vector2f mousePos = Parent->ConvertCoords(input->GetMouseX(), input->GetMouseY()); 
	if(state == CP_ST_MOUSE_LBUTTON_RELEASED && mousePos.x >
		PosX+Width-15)
		state = CP_ST_NONE;
	if(state == CP_ST_NONE)
	{
...
void cpDropDownBox::SetFont(std::string filename)
{
